...
if (
    config.get("secret") == "change"
    and "TEST_ENVIRONMENT" not in os.environ
):
    raise errors.ConfigError(
        "The 'secret' config key is set to the default value 'change'. "
        "You must set a unique, strong secret in config.yaml for "
        "password hashing and token generation security."
    )
