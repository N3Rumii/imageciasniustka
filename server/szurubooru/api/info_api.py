...
def get_info(ctx: rest.Context, _params: Dict[str, str] = {}) -> rest.Response:
    ...
    if auth.has_privilege(ctx.user, "posts:view:featured"):
        # Only include the featured post if the user can actually view it.
        # Private posts are hidden from unauthorized users.
        can_view_featured = (
            post_feature is not None
            and (
                not post_feature.post.is_private
                or posts.try_get_post_by_id(
                    post_feature.post.post_id, ctx.user
                )
                is not None
            )
        )
        ...
        ret["featuringTime"] = (
            post_feature.time if can_view_featured else None
        )
    return ret
