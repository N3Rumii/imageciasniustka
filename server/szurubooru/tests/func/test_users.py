...
def test_update_user_password(user_factory, config_injector):
    config_injector({"password_regex": "^[a-z]+$"})
    user = user_factory()
    with patch("szurubooru.func.auth.generate_salt"), patch(
        "szurubooru.func.auth.get_password_hash"
    ):
        auth.generate_salt.return_value = "salt"
...
def test_reset_user_password(user_factory):
    with patch("szurubooru.func.auth.generate_password"), patch(
        "szurubooru.func.auth.generate_salt"
    ), patch("szurubooru.func.auth.get_password_hash"):
        user = user_factory()
        auth.generate_password.return_value = "generated_pwd"
        auth.generate_salt.return_value = "salt"
        auth.get_password_hash.return_value = ("hash", 3)
        result = users.reset_user_password(user)
        assert result == "generated_pwd"
        assert user.password_salt == "salt"
        assert user.password_hash == "hash"
        assert user.password_revision == 3
