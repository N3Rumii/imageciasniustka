...
class PostAccessDeniedError(errors.AuthError):
    pass
...
def get_post_content_url(post: model.Post) -> str:
    assert post
    # For images, serve the lossy AVIF version if it exists
    if post.type in (model.Post.TYPE_IMAGE, model.Post.TYPE_ANIMATION):
        if files.has(get_post_avif_path(post)):
            return get_post_avif_url(post)
    # For videos, serve AV1 if converted, otherwise original
...
def get_post_avif_url(post: model.Post) -> str:
    assert post
...
def get_post_original_url(post: model.Post) -> str:
    assert post
    original_mime = getattr(post, "_original_mime_type", post.mime_type)
    return "%s/posts/%d_%s.%s" % (
        config.config["data_url"].rstrip("/"),
        post.post_id,
        get_post_security_hash(post.post_id),
...
def get_post_avif_path(post: model.Post) -> str:
    assert post
...
class PostSerializer(serialization.BaseSerializer):
    ...
    def serialize_avif_url(self) -> Any:
        return get_post_avif_url(self.post)
    ...
    def serialize_original_file_format(self) -> Any:
        ...
        if original_mime:
            ext = mime.get_extension(original_mime)
    ...
    def serialize_whitelist(self) -> Any:
        return [
            users.serialize_micro_user(u, self.auth_user)
            for u in self.post.whitelist_users
        ]
...
def _can_view_post(post: model.Post, user: Optional[model.User]) -> bool:
    ...
    return user.user_id in whitelist_ids


def try_get_post_by_id(
    post_id: int, user: Optional[model.User] = None
) -> Optional[model.Post]:
    post = (
    ...
    if post and not _can_view_post(post, user):
        return None
    return post
    ...
    if not _can_view_post(post, user):
        if user and user.user_id:
            raise PostAccessDeniedError(
                "You do not have permission to view post %r." % post_id
            )
        else:
            raise PostAccessDeniedError(
                "You must be logged in to view this private post."
            )
    return post
...
def create_post(
    ...
    if whitelist_user_ids:
        _set_post_whitelist(post, whitelist_user_ids)

    db.session.add(post)
    return post, new_tags


def _set_post_whitelist(post: model.Post, user_ids: List[int]) -> None:
    post.whitelist_users.clear()
    for user_id in user_ids:
        ...
        if user:
            post.whitelist_users.append(user)


def update_post_whitelist(post: model.Post, user_ids: List[int]) -> None:
    assert post
    _set_post_whitelist(post, user_ids)
...
def finalize_post_av1(post: model.Post) -> None:
    """Generate AV1 for video. Call AFTER session flush."""
    _generate_post_av1(post)
    db.session.flush()
...
def _generate_post_avif(post: model.Post) -> None:
    assert post
    content = files.get(get_post_content_path(post))
    if not content:
        return
    if not mime.is_image(post.mime_type):
        return
    try:
        image = images.Image(content)
        avif_cfg = config.config.get("avif", {})
        if mime.is_animated_gif(content):
            avif_data = image.to_animated_avif(
                quality=avif_cfg.get("quality", 50),
                effort=avif_cfg.get("effort", 4),
            )
        else:
            avif_data = image.to_avif(
                quality=avif_cfg.get("quality", 50),
                effort=avif_cfg.get("effort", 4),
            )
...
def _generate_post_av1(post: model.Post) -> None:
    assert post
    if not mime.is_video(post.mime_type):
        return
    content = files.get(get_post_content_path(post))
    if not content:
        return
    try:
        image = images.Image(content)
        av1_cfg = config.config.get("av1", {})
        av1_data = image.to_av1(
