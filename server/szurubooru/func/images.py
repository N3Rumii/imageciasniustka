...
class Image:
    ...
    def to_avif(self, quality: int = 50, effort: int = 4) -> bytes:
        # CRF for libaom-av1: 0-63, lower is better
        # Map quality 0-100 to CRF 63-0
        crf = max(0, min(63, int(63 - (quality / 100.0) * 63)))
        cpu_used = max(0, min(8, effort))
        with util.create_temp_file_path(suffix=".avif") as avif_temp_path:
            self._execute(
                [
                    "-i",
                    "{path}",
            ...
            with open(avif_temp_path, "rb") as avif_temp:
                return avif_temp.read()

    def to_animated_avif(self, quality: int = 50, effort: int = 4) -> bytes:
        # Animated AVIF — same as still but without -still-picture
        crf = max(0, min(63, int(63 - (quality / 100.0) * 63)))
        cpu_used = max(0, min(8, effort))
        with util.create_temp_file_path(suffix=".avif") as avif_temp_path:
            self._execute(
            ...
            with open(avif_temp_path, "rb") as f:
                return f.read()

    def to_av1(self, quality: int = 50, effort: int = 4) -> bytes:
        # SVT-AV1: CRF 1-63, higher = more compression
        # quality 0-100 → CRF 63-30 (0=aggressive, 100=best quality)
        crf = max(30, min(63, int(63 - (quality / 100.0) * 33)))
        # svtav1 preset: 0-13, higher = faster = lower quality
        preset = max(8, min(13, 13 - effort))
        with util.create_temp_file_path(suffix=".webm") as av1_temp_path:
            self._execute(
                [
                    "-i", "{path}",
                    "-c:v", "libsvtav1",
            ...
            with open(av1_temp_path, "rb") as f:
                return f.read()
