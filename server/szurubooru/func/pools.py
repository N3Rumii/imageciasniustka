...
from szurubooru.func import pool_categories, posts, serialization, users, util
...
class PoolAccessDeniedError(errors.AuthError):
    pass
...
class PoolSerializer(serialization.BaseSerializer):
    ...
    def serialize_is_private(self) -> Any:
        return self.pool.is_private

    def serialize_whitelist(self) -> Any:
        return [
            users.serialize_micro_user(u, None)
            for u in self.pool.whitelist_users
        ]
...
def _can_view_pool(
    pool: model.Pool, user: Optional[model.User]
) -> bool:
    """Return whether the user can view a private pool."""
    if not pool.is_private:
        return True
    if not user:
        return False
    whitelist_ids = [u.user_id for u in pool.whitelist_users]
    return user.user_id in whitelist_ids


def try_get_pool_by_id(
    pool_id: int, user: Optional[model.User] = None
) -> Optional[model.Pool]:
    pool = (
    ...
    if pool and not _can_view_pool(pool, user):
        return None
    return pool
...
def get_pool_by_id(
    ...
    if not _can_view_pool(pool, user):
        if user and user.user_id:
            raise PoolAccessDeniedError(
                "You do not have permission to view pool %r." % pool_id
            )
        else:
            raise PoolAccessDeniedError(
                "You must be logged in to view this private pool."
            )
    return pool
...
def update_pool_whitelist(pool: model.Pool, user_ids: List[int]) -> None:
    assert pool
    pool.whitelist_users.clear()
    for user_id in user_ids:
        ...
        if user:
            pool.whitelist_users.append(user)
