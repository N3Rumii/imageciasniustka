...
const GalleryView = require("../views/pool_view.js");
...
class GalleryController {
    constructor(ctx, section) {
        ...
        Promise.all([
            GalleryCategoryList.get(),
            Gallery.get(ctx.parameters.id),
        ]).then(
            (responses) => {
                const [poolCategoriesResponse, pool] = responses;

                topNavigation.activate("pools");
                topNavigation.setTitle("Gallery #" + pool.names[0]);
    ...
    _evtUpdate(e) {
        ...
        e.detail.pool.save().then(
            () => {
                this._view.showSuccess("Gallery saved.");
    ...
    _evtMerge(e) {
        ...
        e.detail.pool.merge(e.detail.targetPoolId, e.detail.addAlias).then(
            () => {
                this._view.showSuccess("Gallery merged.");
                this._view.enableForm();
                router.replace(
                    uri.formatClientLink(
                        "pool",
                        e.detail.targetGalleryId,
                        "merge"
                    ),
                    null,
                    false
    ...
    _evtDelete(e) {
        ...
        e.detail.pool.delete().then(
            () => {
                ...
                ctx.controller.showSuccess("Gallery deleted.");
            },
            (error) => {
                this._view.showError(error.message);
                this._view.enableForm();
...
module.exports = (router) => {
    router.enter(["pool", ":id", "edit"], (ctx, next) => {
        ctx.controller = new GalleryController(ctx, "edit");
    });
    router.enter(["pool", ":id", "merge"], (ctx, next) => {
        ctx.controller = new GalleryController(ctx, "merge");
    });
    router.enter(["pool", ":id", "delete"], (ctx, next) => {
        ctx.controller = new GalleryController(ctx, "delete");
    });
    router.enter(["pool", ":id"], (ctx, next) => {
        ctx.controller = new GalleryController(ctx, "summary");
    });
};
