    ...
    _evtUpdatePost(e) {
        ...
        post.save().then(
            () => {
                ...
                this._view.sidebarControl.enableForm();
                misc.disableExitConfirmation();
                // Also save privacy settings if changed
                if (e.detail.isPrivate !== undefined) {
                    const apiUri = require("../util/uri.js");
                    if (e.detail.isPrivate) {
                        const names = (e.detail.whitelistUsernames || []).slice();
                        api.post(
                            apiUri.formatApiLink("post", post.id, "whitelist"),
                            { userIds: names }
                        ).then((r) => {
