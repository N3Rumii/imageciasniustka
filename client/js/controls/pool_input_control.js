...
const GalleryAutoCompleteControl = require("./pool_auto_complete_control.js");
...
class GalleryInputControl extends events.EventTarget {
    constructor(hostNode, poolList) {
        super();
        this.galleries = poolList;
        ...
        this._poolListNode = editAreaNode.querySelector("ul.compact-galleries");
        ...
        this._autoCompleteControl = new GalleryAutoCompleteControl(
            this._poolInputNode,
            {
                ...
                confirm: (pool) => {
                    ...
                    this.addGallery(pool, SOURCE_USER_INPUT);
                },
                delete: (pool) => {
                    this._poolInputNode.value = "";
                    this.deleteGallery(pool);
                },
                verticalShift: -2,
            }
        );
        ...
        for (let pool of [...this.galleries]) {
            const listItemNode = this._createListItemNode(pool);
            this._poolListNode.appendChild(listItemNode);
        }
    }

    addGallery(pool, source) {
        if (source !== SOURCE_INIT && this.galleries.hasGalleryId(pool.id)) {
            return Promise.resolve();
        }

        this.galleries.add(pool, false);
    ...
    deleteGallery(pool) {
        if (!this.galleries.hasGalleryId(pool.id)) {
            return;
        }
        this.galleries.removeById(pool.id);
...
module.exports = GalleryInputControl;
