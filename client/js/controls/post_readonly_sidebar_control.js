...
class PostReadonlySidebarControl extends events.EventTarget {
    ...
    _installDownloadButtons() {
        const links = this._hostNode.querySelectorAll(".download-convert");
    ...
    _downloadConverted(format) {
        const postId = this._post.id;
        const url = uri.formatApiLink("post", postId, "download") +
        ...
        api.get(url).then((response) => {
            ...
            const blob = new Blob([byteArr], { type: response.mimeType });
