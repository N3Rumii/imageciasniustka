...
class PostEditSidebarControl extends events.EventTarget {
    constructor(hostNode, post, postContentControl, postNotesOverlayControl) {
        ...
        views.replaceContent(
            ...
            template({
                ...
                isUploader: api.user && api.user.name && post._user ? api.user.name === (post._user.name || post._user) : false,
            })
        );

        const privCheckbox = this._hostNode.querySelector("[name=private-post]");
        ...
        this.dispatchEvent(
            new CustomEvent("submit", {
                detail: {
                    ...
                    isPrivate: this._getPrivacyData().isPrivate,
                    whitelistUsernames: this._getPrivacyData().whitelist,
                },
            })
        );
    }
    ...
    _getPrivacyData() {
        const checkbox = this._hostNode.querySelector("[name=private-post]");
        const input = this._hostNode.querySelector("[name=whitelist]");
        return {
            isPrivate: checkbox ? checkbox.checked : false,
            whitelist: input
