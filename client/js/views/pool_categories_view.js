...
const GalleryCategory = require("../models/pool_category.js");

const template = views.getTemplate("pool-categories");
const rowTemplate = views.getTemplate("pool-category-row");

class GalleryCategoriesView extends events.EventTarget {
    constructor(ctx) {
        super();
        this._ctx = ctx;
        ...
        for (let poolCategory of categoriesToAdd) {
            this._addGalleryCategoryRowNode(poolCategory);
        }
        ...
        ctx.poolCategories.addEventListener("add", (e) =>
            this._evtGalleryCategoryAdded(e)
        );

        ctx.poolCategories.addEventListener("remove", (e) =>
            this._evtGalleryCategoryDeleted(e)
        );
    ...
    _addGalleryCategoryRowNode(poolCategory) {
    ...
    _removeGalleryCategoryRowNode(poolCategory) {
        const rowNode = poolCategory._rowNode;
        rowNode.parentNode.removeChild(rowNode);
    }

    _evtGalleryCategoryAdded(e) {
        this._addGalleryCategoryRowNode(e.detail.poolCategory);
    }

    _evtGalleryCategoryDeleted(e) {
        this._removeGalleryCategoryRowNode(e.detail.poolCategory);
    }

    _evtAddButtonClick(e) {
        e.preventDefault();
        this._ctx.poolCategories.add(new GalleryCategory());
    }
...
module.exports = GalleryCategoriesView;
