<div class='readonly-sidebar'>
    <article class='details'>
        ...
        <% if (ctx.post.isPrivate) { %>
            <section class='private-info'>
                <i class='fa fa-lock'></i> Private
                <% if (ctx.post.whitelist && ctx.post.whitelist.length) { %>
                    <ul class='whitelist'>
                        <% for (let u of ctx.post.whitelist) { %>
                            <li><%= ctx.makeUserLink(u) %></li>
                        <% } %>
                    </ul>
                <% } %>
            </section>
        ...
        <% var dlFormat = ctx.post.originalFileFormat || (ctx.post.type === 'video' ? 'MP4' : (ctx.post.type === 'animation' ? 'GIF' : 'PNG')); %>
        <section class='download-dropdown'>
            <span class='download-dropdown-label'>Download:</span>
            <% if (ctx.post.type === 'video') { %>
                <a download href='<%- ctx.post.contentUrl %>'>AV1</a>
                <span class='sep'>|</span>
                <a href='#' class='download-convert' data-format='mp4'>MP4</a>
            <% } else { %>
